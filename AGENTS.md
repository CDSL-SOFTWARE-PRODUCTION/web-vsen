...
- Demand navigation now centers on `DemandWorkspace` (`/ops/demand-workspace`, shortcut `/ops/demand`), with `Order` and `Contract` moved under `/ops/demand/*` plus legacy redirects from `/ops/orders/*` and `/ops/contracts/*`; list/detail/create/edit pages include step hints/chips to reinforce flow order.
